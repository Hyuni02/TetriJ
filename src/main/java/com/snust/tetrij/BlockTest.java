...
public class BlockTest {
    ...
    public void EasyBlockTest() {
        ...
        Controller.field = new ArrayList<Integer>();
        for (int i = 0; i < 7; i++) {
            blockCount.put(i, 0);
        }
        ...
        for (int i = 0; i < totalSelections; i++) {
            Controller.SetField(Tetris.difficulty.EASY);
        }
        for (int i : Controller.field){
            int selectedBlock = i;
        ...
        double actualRate = blockCount.get(1) / (double) Controller.field.size();
        System.out.println("EASY 모드: ");
        System.out.println("총 블록 수: " + Controller.field.toArray().length);
    ...
    public void HardBlockTest() {
        ...
        int totalSelections = 1000;
        for (int i = 0; i < totalSelections; i++) {
            Controller.SetField(Tetris.difficulty.HARD);
        }
        for (int i : Controller.field){
            int selectedBlock = i;
            blockCount.put(selectedBlock, blockCount.get(selectedBlock) + 1);
